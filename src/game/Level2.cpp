...
bool ChatHandler::HandleShowHonor(const char* args)
{
    ...
    {
        rank_name = GetTrinityString(alliance_ranks[ honor_rank ]);
        hrank_name = GetTrinityString(alliance_ranks[ highest_rank ]);
    }
    else
    if ( target->GetTeam() == HORDE )
    {
        rank_name = GetTrinityString(horde_ranks[ honor_rank ]);
        hrank_name = GetTrinityString(horde_ranks[ highest_rank ]);
    }
    else
    {
        rank_name = GetTrinityString(LANG_NO_RANK);
        hrank_name = GetTrinityString(LANG_NO_RANK);
    }
