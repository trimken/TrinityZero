...
bool Guild::LoadGuildFromDB(uint32 GuildId)
{
    ...
    //                                        0        1     2           3            4            5           6
    QueryResult *result = CharacterDatabase.PQuery("SELECT guildid, name, leaderguid, EmblemStyle, EmblemColor, BorderStyle, BorderColor,"
    //   7                8     9     10
...
{
    Field *fields;
    QueryResult *result = CharacterDatabase.PQuery("SELECT rname,rights,rid FROM guild_rank WHERE guildid = '%u' ORDER BY rid ASC", GuildId);
...
{
    //                                                     0                 1     2      3        4
    QueryResult *result = CharacterDatabase.PQuery("SELECT guild_member.guid,rank, pnote, offnote,logout_time FROM guild_member LEFT JOIN characters ON characters.guid = guild_member.guid WHERE guildid = '%u'", GuildId);
